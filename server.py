...
def init_db() -> None:
    with connect() as db:
        ...
        for row in db.execute("select token from sessions where csrf_token = '' or csrf_token is null").fetchall():
            db.execute("update sessions set csrf_token = ? where token = ?", (secrets.token_urlsafe(32), row["token"]))
        db.execute("update users set role = 'owner' where role = '' or role is null")
        db.execute("update users set name = (select owner from stores where stores.id = users.store_id) where name = '' or name is null")
...
def seed_store(db: sqlite3.Connection, store_id: str, store_name: str, owner: str, city: str, plan: str, login: str, password: str) -> None:
    existing = db.execute("select id from stores where store_id = ?", (store_id,)).fetchone()
    if existing:
        return
    ...
    db.execute(
        ...
        (cursor.lastrowid, login.lower(), hash_password(password), owner, "owner"),
    )


def seed_user(db: sqlite3.Connection, store_id: str, login: str, password: str, name: str, role: str) -> None:
    if db.execute("select 1 from users where login = ?", (login.lower(),)).fetchone():
        return
    store = db.execute("select id from stores where store_id = ?", (store_id,)).fetchone()
    if not store:
        return
    db.execute(
        "insert into users (store_id, login, password_hash, name, role) values (?, ?, ?, ?, ?)",
...
def make_store_id(db: sqlite3.Connection, store_name: str) -> str:
    ...
    candidate = base
    index = 2
    while db.execute("select 1 from stores where store_id = ?", (candidate,)).fetchone():
...
class BloomHandler(SimpleHTTPRequestHandler):
    ...
    def handle_api_post(self) -> None:
        ...
        if path == "/api/form-register":
            ...
            with connect() as db:
                existing_user = db.execute("select 1 from users where login = ?", (login,)).fetchone()
                if existing_user:
                ...
                user_cursor = db.execute(
                    "insert into users (store_id, login, password_hash, name, role) values (?, ?, ?, ?, ?)",
                    (cursor.lastrowid, login, hash_password(password), owner, "owner"),
                )
                db.commit()
                user = db.execute(
                    """
        ...
        if path == "/api/users":
            ...
            with connect() as db:
                ...
                created = db.execute("select id, login, name, role from users where id = ?", (cursor.lastrowid,)).fetchone()
            data = load_store_data(user)
